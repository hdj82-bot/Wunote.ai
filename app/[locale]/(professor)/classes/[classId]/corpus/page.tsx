      ...
      <div className="flex items-baseline justify-between">
        <div>
          ...
          <h1 className="text-lg font-bold text-slate-900">
            {classInfo.name} <span className="text-sm font-normal text-slate-500">({classInfo.semester})</span>
          </h1>
        </div>
        <p className="text-xs text-slate-500">
          {docs.length} / {MAX_DOCS_PER_CLASS}
        </p>
      </div>

      <section className="space-y-2">
        <h2 className="text-sm font-semibold text-slate-700">새 파일 업로드</h2>
        <CorpusUploader
          classId={params.classId}
          disabled={atLimit}
          disabledReason={atLimit ? `수업당 최대 ${MAX_DOCS_PER_CLASS}개까지 업로드할 수 있습니다.` : undefined}
        />
      </section>

      <section className="space-y-2">
        <h2 className="text-sm font-semibold text-slate-700">업로드된 코퍼스</h2>
        {docs.length === 0 ? (
                ...
                <Card className="flex items-center gap-3 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      {d.is_public && <span className="ml-2 text-indigo-600">· 공개</span>}
                      {d.download_count > 0 && (
                        <span className="ml-2">· 다운로드 {d.download_count}회</span>
                      )}
                    </p>
                  </div>
                </Card>
